...
// _Backend/tasbeehbackend.ts
import { doc, onSnapshot, setDoc } from 'firebase/firestore';
import { auth, db } from '../firebaseConfig';

export const syncZikrToCloud = async (zikrId: string, count: number, arabic: string) => {
  if (!auth.currentUser) return;
  const ref = doc(db, 'users', auth.currentUser.uid, 'tasbeeh', zikrId);
  await setDoc(ref, { count, arabic, updatedAt: Date.now() }, { merge: true });
};
...
export const listenToZikrChanges = (zikrId: string, callback: (count: number) => void) => {
  ...
  return onSnapshot(ref, (doc) => {
    if (doc.exists()) callback(doc.data().count);
  });
};