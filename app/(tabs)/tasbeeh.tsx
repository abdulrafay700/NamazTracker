...
// import { useNavigation } from '@react-navigation/native'; // Back detection ke liye

// // export default function TasbeehApp() {
// //     const navigation = useNavigation(); // Navigation hook
    
// //     // --- States (Pehle se majood) ---
// //     const [zikrs, setZikrs] = useState<Zikr[]>(INITIAL_ZIKRS);
// //     const [showSyncToast, setShowSyncToast] = useState(false);
// //     // ... baaki saari states

// //     // --- Toast Animation Values (Exactly as you wanted) ---
...
// //         toastOpacity.value = withTiming(1, { duration: 500 });
// //         toastTranslateY.value = withTiming(0, { duration: 500 });

// //         setTimeout(() => {
// //             toastOpacity.value = withTiming(0, { duration: 500 });
// //             toastTranslateY.value = withTiming(-20, { duration: 500 }, () => {
// //                 // Animation khatam hone par state reset
// //                 // setShowSyncToast(false); // Ye callback UI thread par hota hai
// //             });
// //         }, 2500);
// //     };

// //     // --- AUTOMATIC SYNC ON BACK (New Logic) ---
// //     useEffect(() => {
// //         // Jab user screen se "blur" ho (yaani piche jaye)
// //         const unsubscribe = navigation.addListener('blur', () => {
// //             if (auth.currentUser) {
// //                 // Yahan aapka data pehle hi sync ho raha hai handlePress mein, 
// //                 // isliye hum sirf confirmation toast dikhayenge
// //                 triggerSyncToast();
// //             }
// //         });
...
// //             {/* --- CUSTOM SYNC TOAST (Top Fixed) --- */}
// //             {showSyncToast && (
// //                 <Animated.View style={[styles.customSyncToast, animatedToastStyle]}>
// //                     <BlurView intensity={80} tint="dark" style={styles.toastBlurContent}>
// //                         <CheckCircle2 size={18} color="#10b981" />
// //                         <Text style={styles.toastTextContent}>Cloud Synced Successfully</Text>
// //                     </BlurView>
// //                 </Animated.View>
// //             )}

// //             {/* ... Aapka baaki poora UI (Header, Circle, Button etc) waise hi rahega ... */}
            
// //             {/* ... (Copy from your existing code from <Animated.View style={[styles.toast, animatedToastStyle]}> onwards) */}
...
export default function TasbeehApp() {
    ...
    const toggleSound = (val: boolean) => {
        setIsSoundEnabled(val);
        if (auth.currentUser) setDoc(doc(db, 'users', auth.currentUser.uid, 'settings', 'lastSession'), { sound: val }, { merge: true });
    };

    const toggleVibration = (val: boolean) => {
        setIsVibrationEnabled(val);
        if (auth.currentUser) setDoc(doc(db, 'users', auth.currentUser.uid, 'settings', 'lastSession'), { vibration: val }, { merge: true });
    };
            ...
            <Modal visible={showGoalModal} transparent animationType="fade">
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        ...
                        <TextInput placeholder="Target..." placeholderTextColor="#444" keyboardType="numeric" style={styles.input} value={customGoal} onChangeText={setCustomGoal} />
                        <TouchableOpacity style={styles.doneBtn} onPress={() => { if(customGoal){ updateTarget(parseInt(customGoal)); setShowGoalModal(false); setCustomGoal(''); } }}>
